...
export default function HeroSection() {
  ...
  return (
    <section className="min-h-screen flex items-center justify-center pt-20 px-6 md:px-12 relative overflow-hidden">
      ...
      >
        <span className="text-[10px] uppercase tracking-[0.2em] text-secondary group-hover:text-accent transition-colors">Scroll Down</span>
        <div className="w-6 h-10 border-2 border-secondary group-hover:border-accent rounded-full relative transition-colors">
          <motion.div 
            animate={{ y: [0, 12, 0] }}
            transition={{ duration: 1.5, repeat: Infinity, ease: "easeInOut" }}
            className="w-1 h-2 bg-accent rounded-full absolute top-2 left-1/2 -translate-x-1/2"
          />
        </div>
      </motion.div>
    </section>
